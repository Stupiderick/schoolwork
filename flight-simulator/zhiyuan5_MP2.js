...
function drawTerrain(){
 ...
 //Draw
    ...
    //gl.drawArrays(gl.TRIANGLES, 0, tVertexPositionBuffer.itemSize)
}
...
function draw() {
    ...
    window.addEventListener("keydown", function (event) {
        ...
        switch (event.key) {
            ...
            // case "<":
            // quat.fromEuler(tempQuat, -0.5 * Math.sin(angle), 0.0, 0.0);
            // quat.invert(invQuat, tempQuat);
            // quat.multiply(curViewDir, tempQuat, quat.multiply(curViewDir, curViewDir, invQuat));
            // viewDir = vec3.fromValues(curViewDir[0], curViewDir[1], curViewDir[2]);
            // //console.log(up);
...
function setPhongNoFogShader() {
    console.log("Setting Phong shader");
    setupShaders("shader-phong-phong-vs","shader-phong-phong-nofog-fs");
}
...
function startup() {
    ...
    document.getElementById("phong-phong-no-fog").onclick = setPhongNoFogShader;
