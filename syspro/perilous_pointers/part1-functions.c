...
float *four(const int *x) {
    // POINT the ptr to heap, by using malloc().
...
void five(const char *a) {
    // CONVERT a into *a to get the first char in a
    if ((*a >= 'A' && *a <= 'Z') || (*a >= 'a' && *a <= 'z'))
