...
void document_write_to_file(document *this, const char *path_to_file) {
    ...
    FILE *myfile = fopen(path_to_file, "w+");

    for (int i = 0; i < (int)vector_size(this->vector); i++) {
        char *line = vector_get(this->vector, i);
        fprintf(myfile, "%s\n", line);
    }
...
document *document_create_from_file(const char *path_to_file) {
    ...
    char *buf = NULL;
    size_t bufSize = 0;
    while (getline(&buf, &bufSize, myfile) != -1) {
        char *checker;
        if ((checker = strchr(buf, '\n')) != NULL)
            *checker = '\0';
        vector_push_back(ret->vector, buf);
    }
...
void document_insert_line(document *this, size_t line_number, const char *str) {
    ...
    size_t maxLine = document_size(this);

    // when insert the line at the middle of the file...
    if (line_number <= maxLine) {
        vector_insert(this->vector, line_number - 1, (char *)str);
    // or it exceeds the maximum of the line number...
    } else {
        for (size_t i = maxLine + 1; i < line_number; i++) {
            vector_push_back(this->vector, "");
        }
        vector_push_back(this->vector, (char *)str);
    }
}
