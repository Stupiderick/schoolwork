 ...
 * @return {number}
 */
function terrainGenerator(n, minX, maxX, minY, maxY, vertexArray, faceArray,
    ...
    heightCheck[0] = true;
    heightCheck[n] = true;
    heightCheck[n * (n + 1)] = true;
    heightCheck[(n + 1) ** 2 - 1] = true;

    generateDiamondSquare(n, vertexArray, heightCheck);
    normalizeHeight(n, vertexArray, 0.5, -0.5);
...
/**
 * generate the height of the mid point.
 * @param {number} n
 * @param {Array} vertexArray Array that will contain vertices generated
 * @param {Array} faceArray Array that will contain faces Generated
 */
function generateDiamondSquare(n, vertexArray, heightCheck) {
    var lv = 1; // level
    for (var i = 0; i < Math.log2(n); i++) {
        // Diamond part
        ...
        for (var j = 0; j < 4 ** (lv - 1); j++) {
            getMidPointHeight(n, p1, p2, p3, p4, vertexArray, heightCheck, lv);
            if ((p2 + 1) % (n + 1) == 0) {
                // when the block is supposed to change line.
                p1 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
                p2 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
                p3 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
                p4 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
            } else {
                p1 += (n / (2 ** (lv - 1)));
                p2 += (n / (2 ** (lv - 1)));
                p3 += (n / (2 ** (lv - 1)));
        ...
        for (var j = 0; j < 4 ** (lv - 1); j++) {
            getNeighborsHeight(n, p1, p2, p3, p4, vertexArray, heightCheck, lv);
            if ((p2 + 1) % (n + 1) == 0) {
                p1 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
                p2 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
                p3 += ((n + 1) * n / (2 ** (lv - 1))) - (n - n / (2 ** (lv - 1)));
        ...
        lv += 1; // getting into the next lv (smaller blocks)
    }
}
