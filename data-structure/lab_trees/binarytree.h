...
class BinaryTree
{
  ...
  private:
     ...
     * @param subRoot The current node in the recursion
     * @param vec The path of the tree
     */
    void printPaths(Node * subRoot, vector<T> & vec) const;

    /**
     * Private helper function for the public sumDistances function.
     * @param subRoot The current node in the recursion
     * @param subSum The current sum of the distance
     * @param level Record of the reversed level which the subRoot is pointing
     *  at. For instance, the root is pointing at level height(root), and the
     *  subRoot of root is pointing at level height(root) - 1, and so on and
     *  so for.
     */
    int sumDistances(Node * subRoot, int subSum, int level) const;
