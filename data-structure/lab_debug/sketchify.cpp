...
void sketchify() {
    // Load in.png
    PNG* original = new PNG();  //add "new PNG()"
    ...
    // Create out.png
    PNG* output = new PNG(width, height);  //add "new PNG(width, height)"
    //delete setupOutput
    ...
    for (int y = 1; y < height; y++) {    //delete "0 <", so does for the next line.
        for (int x = 1; x < width; x++) {
            ...
            if (diff > 100)
                *currOutPixel = *myPixel;    //change these to pointers
        }
    }
