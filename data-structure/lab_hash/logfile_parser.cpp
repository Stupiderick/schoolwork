...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
         ...
         * vector as well.
         */
        string helperKey = ll.url;
        string theCustomer = ll.customer.substr(1, ll.customer.length() - 3);
        helperKey.append(theCustomer);

        if (whenVisitedTable.keyExists(helperKey)) {
            if (whenVisitedTable[helperKey] < ll.date) {
                whenVisitedTable[helperKey] = ll.date;
            }
        } else {
            whenVisitedTable[helperKey] = ll.date;
        }
...
{
    string helperKey = url;
    string theCustomer = customer.substr(1, customer.length() - 3);
    helperKey.append(theCustomer);

    return whenVisitedTable.keyExists(helperKey);
}
                                  ...
                                  const string& url) const
{
    ...
    time_t lastDate = whenVisitedTable.find(helperKey);
