...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     /**
      * If the singleQueue is empty, we should return it directly!
      */
     ...
     /**
      * If there is only one TreeNode inside the singleQueue, we can assign it
      * as the root of tree directly.
      */
     ...
     while (!(singleQueue.empty() && mergeQueue.size() == 1)) {
        ...
        /**
         * Here we are gonna merge the temps, and then put it into the
         * mergeQueue, plusing the freq of the two temps together!
         */
        TreeNode * tempMerge = new TreeNode(temp1->freq.getFrequency() +
                                            temp2->freq.getFrequency());
        tempMerge->left = temp1;
        tempMerge->right = temp2;

        mergeQueue.push(tempMerge);
     }
