...
class KDTree
{
  ...
  private:
    ...
    /**
     * Helper function for constructor.
     * Select the middle value, and put it to the middle of the vector.
     */
    void select(int left, int right, int k, int dimension);

    /**
     * Helper function for constructor.
     * Find the place that need to break the vector into partitions.
     */
    int partition(int left, int right, int pivotIndex, int dimension);

    /**
     * Helper function for constructor.
     * Swap two Points within a vector.
     */
     ...
     * Helper function
     */
    Point<Dim> findLeaf(const Point<Dim>& query, Point<Dim> currClosest,
                    int left, int right, int dimension, bool& searching) const;

    /**
     * Helper function
     */
    int findDistance(const Point<Dim>& target, const Point<Dim>& sample) const;
};
