...
class AVLTree
{
  ...
  private:
    ...
    /**
     * Private helper function for the case that the node waiting for being
     * removed has no children.
     */
    void noChildRemove(Node*& subtree);

    /**
     * Private helper function for the case that the node waiting for being
     * removed has only one child.
     */
    void oneChildRemove(Node*& subtree);

    /**
     * Private helper function for the case that the node waiting for being
     * removed has two children.
     */
    void twoChildRemove(Node*& subtree);

    /**
     * Private helper function for finding the rightmost child whem we are
     * dealing the two-child situation that we need to swap the data.
     * return the pointer to the Node that is the rightmost child.
     */
    Node * rightMostChild(Node* subtree);

    /**
     * Private helper function for renewing the height in the case of two-child.
     */
    void renewHeight(Node* from, Node* upto);
