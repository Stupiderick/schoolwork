...
{
    /** The case that elements is empty, then the index will be at 0! */
    if (elements.empty()) {
        return 0;
    }

    int sizeOfElements = elements.size(); /** Get the size of the vector */

    /**
     * The case that the val is smaller than the val at the first index of
     * vector, we need to return 0 directly, because the new val will be at the
     * index 0.
     */
    if (elements.at(0) > val) {
        return 0;
    }

    /**
     * The case that the val is greater than the val at the last index of
     * vector, we need to put it to the end of the vector directly, so we
     * return size of the vector as the index.
     */
    if (elements.at(sizeOfElements - 1) < val) {
    ...
    /**
     * Use binary search to find the place for the new val, when upper - lower
     * smaller than 1, then we need to consider the case that val is not in the
     * original vector!
     */
    while (upper - lower > 1) {
        ...
        if (elements.at(middle) == val) {
            return middle;
        } else if (elements.at(middle) < val) {
            lower = middle;
        } else {
            upper = middle;
        }
...
// template <class T, class C>
// size_t insertion_idx(const std::vector<T>& elements, const C& val, int lower, int upper) {
//     if (upper - lower == 1) {
//         return upper;
//     }
//
...
//         return middle;
//     } else if (elements.at(middle) < val) {
//         return insertion_idx(elements, val, middle, upper);
//     } else {
//         return insertion_idx(elements, val, lower, middle);
//     }
// }
