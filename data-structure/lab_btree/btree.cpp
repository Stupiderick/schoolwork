...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    unsigned sizeOfElements = subroot->elements.size();

    if (first_larger_idx < sizeOfElements) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->is_leaf) {
        auto elem_insert = subroot->elements.begin() + first_larger_idx;
        subroot->elements.insert(elem_insert, pair);
        subroot->children.push_back(NULL);
        return;
    } else {
