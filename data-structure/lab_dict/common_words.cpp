...
void CommonWords::init_common()
{
    for (size_t i = 0; i < file_word_maps.size(); i++) {
        for (auto & insideFile : file_word_maps[i]) {
            auto lookup = common.find(insideFile.first);
            if (lookup != common.end()) {
                lookup->second++;

            } else {
                common[insideFile.first] = 1;
            }
        }
    }
}
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for (auto & key : common) {
        ...
        if (key.second == file_word_maps.size()) {
            for (auto & insideFile : file_word_maps) {
                if (insideFile.find(key.first)->second < n) {
                    check = false;
                    break;
                }
            }
